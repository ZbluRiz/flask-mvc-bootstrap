...
class Book:
    ...
    @staticmethod
    def get_all_books(page=1, per_page=10, sort_by='title', sort_order='asc'):
        db = Database()
    ...
    def search_books(query):
        db = Database()
        db.cursor.execute("SELECT * FROM books WHERE title LIKE %s OR author LIKE %s OR isbn LIKE %s", 
                          ('%' + query + '%', '%' + query + '%', '%' + query + '%'))
        books = db.cursor.fetchall()
        db.close()
        return books
